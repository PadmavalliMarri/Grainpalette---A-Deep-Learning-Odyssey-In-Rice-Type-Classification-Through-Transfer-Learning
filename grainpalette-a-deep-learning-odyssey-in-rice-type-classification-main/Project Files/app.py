...
CONFIDENCE_THRESHOLD = 0.6  # 60% minimum confidence required
MODEL_PATH = 'best_rice_model.h5'
model = None
...
def predict_rice_type(img):
    """Predict rice type from image using the trained model"""
    if model is None:
        return None, None
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            if predicted_class is None or confidence < CONFIDENCE_THRESHOLD:
                flash('Low confidence in prediction. Please upload a proper rice grain image.')
                return redirect(url_for('index'))
